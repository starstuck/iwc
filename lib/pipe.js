...
define(function () {
	...
	function dispatchMessage(event) {
		...
		try {
			...
			if (err.type !== 'MessageDecodingError') {
				...
				if (ignoredMessagePrefixes.filter(function (prefix) {
					...
					console.info('iwc.pipe.MessageDispatcher', 'ignored not prefixed message: ', message);
				}
				// #endif
				return;
			}
		...
		// #ifdef DEBUG
		console.debug('iwc.pipe.MessageDispatcher', 'at ' + localContextUri + ' dispatching: ', message);
		// #endif
		...
		// #ifdef DEBUG
		if (!processed) {
			console.warn('iwc.pipe.MessageDispatcher', 'did not found recipient for: ', event.data);
	...
	function getContextByUri(contextUri) {
		...
		if (!context) {
			...
			// #ifdef debug
			console.debug('idc.pipe.getContextByUri', 'building new frame:', contextUri);
			// #endif
