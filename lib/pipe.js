...
define(function () {
	...
	function PipeUri(contextUri, name, destiny) {
	 ...
	 * Local pipes have local destiny
	 */
	function expandPipeUri(uri) {
		var loc = localContext.location,
			match,
			result,
			name,
			destiny,
		...
		if (pipeNameRe.exec(uri)) {
			name = uri;
			contextUri = null;
			destiny = loc.protocol + '//' + loc.host + (loc.port ? ':' + loc.port : '');
		} else {
			match = pipeUriRe.exec(uri);
			if (match) {
				contextUri = (match[1] || loc.protocol) + '//' + match[2];
				name = match[5] || DEFAULT_PIPE_NAME;
				destiny = (match[1] || loc.protocol) + '//' + match[3];
			} else {
				throw new Error('Invalid pipe uri: ' + uri);
			}
		}
		return new PipeUri(contextUri, name, destiny);
	}
	 ...
	 */
	function MessagePipe(name, context, destiny) {
		var me = this;
		...
		if (destiny) {
			...
			me.destiny = '*';
			// #ifdef DEBUG
			log(WARN, 'iwc.pipe.MessagePipe created without target destiny, which is huge security risk: ', name);
			// #endif
		}
	...
	MessagePipe.prototype = {
		...
		send: function (data) {
			if (this.context) {
				...
				// #endif
				this.context.postMessage(encodeMessage(this.name, data), this.destiny);
			} else {
				this._push(data);
			}
			return this;
	...
	pub.open = function (uri, context) {
		...
		pipe = new MessagePipe(pipeUri.name, context, pipeUri.destiny);
	...
	pub.addRecipient = function (uri, handler, allowedOrigins) {
		...
		// #ifdef DEBUG
		log(DEBUG, 'iwc.pipe.MessageDispatcher has new reciver for: ', pipeUri, allowedOrigins);
		if (!allowedOrigins) {
			log(WARN, 'iwc.pipe.MessageDispatcher got reciver with undefined message origin filter. This is serious security risk: ', pipeUri);
		}
		// #endif
