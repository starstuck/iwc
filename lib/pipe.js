...
define(function () {
	...
	function PipeUri(contextUri, name, origin) {
	 ...
	 */
	function MessagePipe(name, context, origin) {
		var me = this;
		...
		if (origin) {
			...
			me.origin = '*';
			// #ifdef DEBUG
			log(WARN, 'iwc.pipe.MessagePipe created without target origin, which is huge security risk: ', name);
			// #endif
		}
	...
	MessagePipe.prototype = {
		...
		send: function (data) {
			if (this.context) {
				...
				// #endif
				this.context.postMessage(encodeMessage(this.name, data), this.origin);
			} else {
				this._push(data);
			}
			return this;
	...
	pub.open = function (uri, context) {
		...
		pipe = new MessagePipe(pipeUri.name, context, pipeUri.origin);
